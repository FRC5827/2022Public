...
public class SwerveModule implements Sendable {
    ...
    private WPI_CANCoder m_angleEncoder;
    private double m_lastAngleInTicks;

    // for simulation
    public TalonFXSimCollection m_angleMotorSim;
    public TalonFXSimCollection m_driveMotorSim;
    public FlywheelSim m_angleWheelSim;
    public FlywheelSim m_driveWheelSim;

    private final SimpleMotorFeedforward m_feedForward = new SimpleMotorFeedforward(Constants.Swerve.driveKS, Constants.Swerve.driveKV, Constants.Swerve.driveKA);

    // Target SwerveModuleState expressed as doubles to reduce garbage collections.
    ...
    public void setDesiredState(SwerveModuleState desiredState, boolean isOpenLoop) {
        ...
        if (!bUseLastVelocity) {
            if (isOpenLoop) {
                ...
                m_driveMotor.set(TalonFXControlMode.Velocity, velocity, DemandType.ArbitraryFeedForward, m_feedForward.calculate(desiredState.speedMetersPerSecond) / Constants.General.kNominalBatteryVoltage);
        ...
        m_targetVelocity = desiredState.speedMetersPerSecond;
        m_targetAngle = desiredState.angle.getDegrees();
    }
    ...
    private void resetToAbsolute() {
        ...
        CTREConfigs.checkCtreError(m_angleMotor.setSelectedSensorPosition(absolutePosition, 0, Constants.General.kCANConfigTimeout), "Error setting angle Talon " + m_number + " sensor position");
    }

    private void configAngleEncoder() {        
        CTREConfigs.configCANCoderFactoryDefault(m_angleEncoder, "Error configuring CANcoder " + m_number + " default settings");
        CTREConfigs.setCANCoderStatusFramePeriod(m_angleEncoder, CANCoderStatusFrame.SensorData, Constants.General.kTalonCANStatusSlowRateInMs, "Error setting CANCoder " + m_number + " SensorData period");
        CTREConfigs.checkCtreError(m_angleEncoder.configAllSettings(Robot.m_ctreConfigs.swerveCanCoderConfig, Constants.General.kCANConfigTimeout), "Error configuring CANCoder " + m_number + " configuration");
    }
    ...
    private void configAngleMotor() {
        ...
        CTREConfigs.setTalonStatusFramePeriod(m_angleMotor, StatusFrameEnhanced.Status_1_General, Constants.General.kTalonCANStatusVerySlowRateInMs - (15 * m_number), "Error setting angle Talon " + m_number + " status_1 period");

        CTREConfigs.checkCtreError(m_angleMotor.configAllSettings(Robot.m_ctreConfigs.swerveAngleFXConfig, Constants.General.kCANConfigTimeout), "Error configuring angle Talon " + m_number + " configuration");
        CTREConfigs.checkCtreError(m_angleMotor.configVoltageCompSaturation(Constants.General.kNominalBatteryVoltage, Constants.General.kCANConfigTimeout), "Error configuring angle Talon " + m_number + " voltage comp saturation"); 
    ...
    private void configDriveMotor(boolean invertDriveMotor) {  
        ...
        // reduce CAN usage since we don't need general status data as frequently
        CTREConfigs.setTalonStatusFramePeriod(m_driveMotor, StatusFrameEnhanced.Status_1_General, Constants.General.kTalonCANStatusVerySlowRateInMs - (15 * m_number), "Error setting drive Talon " + m_number + " status_1 period");
        // increase CAN status frames for sensor position for better accuracy
        CTREConfigs.setTalonStatusFramePeriod(m_driveMotor, StatusFrameEnhanced.Status_2_Feedback0, Constants.General.kTalonCANStatusFastRateInMs, "Error setting drive Talon " + m_number + " status_2 period");

        CTREConfigs.checkCtreError(m_driveMotor.configAllSettings(Robot.m_ctreConfigs.swerveDriveFXConfig, Constants.General.kCANConfigTimeout), "Error configuring drive Talon " + m_number + " configuration");
        CTREConfigs.checkCtreError(m_angleMotor.configVoltageCompSaturation(Constants.General.kNominalBatteryVoltage, Constants.General.kCANConfigTimeout), "Error configuring drive Talon " + m_number + " voltage comp saturation"); 
        ...
        CTREConfigs.checkCtreError(m_driveMotor.setSelectedSensorPosition(0, 0, Constants.General.kCANConfigTimeout), "Error setting drive Talon " + m_number + " sensor position");
